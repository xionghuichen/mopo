...
def main():
    ...
    for ind, (r_conservative, r_optimal, r_adaptable) in enumerate(zip(rewards_conservative,
        ...
        plt.cla()
        plt.plot(np.array([i for i in range(length)]).reshape((-1, 1)), r_adaptable, '-+', label='probe-reduce')
        plt.plot(np.array([i for i in range(length)]).reshape((-1, 1)), r_adaptable, '-x', label='adaptable')
        plt.plot(np.array([i for i in range(length)]).reshape((-1, 1)), r_conservative, '-^', label='conservative')

        plt.plot(np.array([i for i in range(length)]).reshape((-1, 1)), r_optimal, '-*', label='optimal')

        plt.xlim(left=-0.2, right=length-1 + 0.2)
        plt.xlabel('step')
        plt.ylabel('reward')
        plt.legend()
