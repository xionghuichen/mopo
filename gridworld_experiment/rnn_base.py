...
class RNNBase(torch.nn.Module):
    def __init__(self, input_size, output_size, hidden_size_list, activation, layer_type, logger=None, aux_dim=0):
        ...
        for ind, item in enumerate(hidden_size_list):
            if self.layer_type[ind] == 'fc':
                ...
                self.rnn_num += 1
                self.layer_list.append(layer_dict[self.layer_type[ind]](lst_nh, item, batch_first=True))
        ...
        if self.layer_type[-1] == 'fc':
            ...
            self.rnn_num += 1
            self.layer_list.append(layer_dict[self.layer_type[-1]](lst_nh, output_size, batch_first=True))
    ...
    @staticmethod
    def get_hidden_lenght(hidden_state):
